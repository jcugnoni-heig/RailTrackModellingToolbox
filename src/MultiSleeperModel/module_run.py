...
def RunPhase1(p_dictSimu):
	...
	code = SaveBaseFiles(cwd, saveBaseFile, simFolder, nJobs)
	if code != 0:
		return code
...
def PrepareFilesPhase1(p_dictSimu):
	...
	try:
		...
		asterFilesPath = os.path.join(p_dictSimu['cwd'], 'AsterFiles')
	...
	try:
		...
		os.system('sed -i -E "s!__cwd__!' + p_dictSimu['cwd'] + '!" ' + exportFiles)
...
def SaveBaseFiles(p_cwd, p_saveBaseDir, p_simFolder, p_nJobs):
...
def PostProcessResults(p_dictSimu):
	...
	try:
		shutil.copyfile(os.path.join(cwd, 'AsterFiles', 'postPro_concatMedFiles.comm'), os.path.join(fullDir, 'postPro_concatMedFiles.comm'))
		
		postProExportFile = os.path.join(fullDir, 'postPro_concatMedFiles1.export')
		shutil.copyfile(os.path.join(cwd, 'AsterFiles', 'postPro_concatMedFiles1.export'), postProExportFile)
	except:
	...
	try:
		...
		os.system('sed -i -E "s!__cwd__!' + p_dictSimu['cwd'] + '!" ' + exportFile)
